...
def new_view(request):
    ...
    # post 요청이 들어오면 request.POST 변수를 받아서 글 생성
    if request.method == 'POST':
        title = request.POST.get('title', None)
        content = request.POST.get('content', None)
        # '영화' 라는 카테고리로 글을 썼다면 category_name 에는 '영화'가 할당 될 것
        ...
        # django 는 자동으로 PK 라는 필드를 만들어 준다.
        # 첫번째 글을 썼다 -> pk = 1
        # 두번째 글을 썼다 -> pk = 2
        return redirect('detail', article.pk)

    elif request.method == 'GET':
        # 새 글을 쓸 때 현재 category 모델에 존재하는 것만 사용자가 선택할 수 있어야 함.
